      ...
      <div className="container mx-auto px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-md mx-6">
            <div className="relative w-full group">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-slate-800/60 border border-white/10 text-white placeholder:text-slate-500 focus:outline-none focus:border-green-500/50 focus:bg-slate-800/80 transition-all text-sm"
              />
            </div>
          </form>
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-3">
            ...
            {/* Currency Toggle */}
            <div className="flex bg-slate-800/60 rounded-lg p-0.5 lg:p-1 border border-white/10">
              <button 
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="md:hidden mt-4 pb-4 pt-4 border-t border-white/10 space-y-3 animate-in slide-in-from-top-2 duration-200">
            {/* Mobile Search */}
            <form onSubmit={handleSearch} className="relative">
              ...
              <input 
                ...
                value={searchQuery} 
                onChange={(e) => setSearchQuery(e.target.value)} 
                className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-slate-800/60 border border-white/10 text-white placeholder:text-slate-500 text-sm" 
              />
            </form>

            {/* Currency & Theme */}
            <div className="flex items-center justify-between">
              <div className="flex bg-slate-800/60 rounded-lg p-0.5 border border-white/10">
                <button 
                  onClick={() => setCurrency('USD')} 
              ...
              <button 
                onClick={() => setTheme(theme === "dark" ? "light" : "dark")} 
                className="p-2 rounded-lg bg-slate-800/60 border border-white/10 text-slate-400"
              >
            ...
            <div className="space-y-1 pt-2">
              <Link 
                ...
                className="block px-3 py-2.5 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800/60 transition-colors"
              >
                Marketplace
              </Link>
              <Link 
            ...
            {user ? (
              <div className="space-y-2 pt-2 border-t border-white/10">
                {isAdmin() && (
                  <Link 
                    to="/admin" 
                    onClick={() => setIsOpen(false)}
                    className="flex items-center gap-2 px-3 py-2.5 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800/60"
                  >
                ...
                <Button 
                  onClick={handleSignOut} 
                  variant="ghost" 
                  className="w-full text-slate-400 hover:text-white hover:bg-slate-800/60"
                >
              ...
              <div className="flex gap-2 pt-2 border-t border-white/10">
                <Button asChild className="flex-1 bg-slate-800/60 hover:bg-slate-700 text-white py-2.5 rounded-lg border border-white/10">
